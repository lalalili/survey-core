...
class ValidateSurveyBuilderSchemaAction
{
    ...
    public function execute(array $schema): array
    {
        ...
        $validator = Validator::make($schema, [
            ...
            'settings.category' => ['nullable', 'string', 'max:10'],
            'settings.starts_at' => ['nullable', 'string'],
            'settings.ends_at' => ['nullable', 'string'],
            'settings.close_at' => ['nullable', 'string'],
            'settings.submit_success_message' => ['nullable', 'string'],
            'settings.max_responses' => ['nullable', 'integer', 'min:1'],
            'settings.quota_message' => ['nullable', 'string'],
            'settings.uniqueness_mode' => ['nullable', 'string', Rule::in(array_map(fn (SurveyUniquenessMode $mode): string => $mode->value, SurveyUniquenessMode::cases()))],
            'settings.uniqueness_message' => ['nullable', 'string', 'max:255'],
            'settings.personalization' => ['nullable', 'array'],
            'settings.personalization.audience_list_id' => ['nullable'],
            'settings.personalization.required' => ['nullable', 'boolean'],
            ...
            'settings.personalization.result_context_columns.vehicle_plate' => ['nullable', 'string', 'max:255'],
            'settings.personalization.result_context_columns.delivery_date' => ['nullable', 'string', 'max:255'],
            ...
            'settings.anomaly.turnstile' => ['nullable', 'boolean', function (string $attribute, mixed $value, callable $fail): void {
                // 僅在伺服器已設定 Turnstile 金鑰時才允許開啟，避免「開了卻沒金鑰 → verify 恆 false → 全部送出被擋」。
                if (filter_var($value, FILTER_VALIDATE_BOOLEAN) && blank(config('survey-core.turnstile.secret_key'))) {
            ...
            'pages.*.elements.*.validation_rules.min_length' => ['nullable', 'integer', 'min:0'],
            'pages.*.elements.*.validation_rules.min_chinese_length' => ['nullable', 'integer', 'min:0'],
            'pages.*.elements.*.validation_rules.max_length' => ['nullable', 'integer', 'min:0'],
    ...
    private function validateElements(array $schema): void
    {
        ...
        foreach ($schema['pages'] as $pageIndex => $page) {
            ...
            foreach ($page['elements'] as $elementIndex => $element) {
                ...
                if ((bool) ($element['is_hidden'] ?? false) && blank($element['personalized_key'] ?? null)) {
                    $errors["{$path}.personalized_key"][] = '已勾選個性化欄位，請設定對應名單欄位。';
                }

                foreach (($element['show_if']['conditions'] ?? []) as $conditionIndex => $condition) {
                    if (! is_array($condition)) {
                        continue;
                    }

                    $conditionPath = "{$path}.show_if.conditions.{$conditionIndex}";
                    $operator = (string) ($condition['op'] ?? 'equals');

                    if (blank($condition['field_key'] ?? null)) {
                        $errors["{$conditionPath}.field_key"][] = '顯示條件請選擇目標題目。';
                    }

                    if (! in_array($operator, ['is_empty', 'is_not_empty'], true) && blank($condition['value'] ?? null)) {
                        $errors["{$conditionPath}.value"][] = '顯示條件請填寫輸入值。';
                    }
                }

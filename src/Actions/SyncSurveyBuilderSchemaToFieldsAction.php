...
class SyncSurveyBuilderSchemaToFieldsAction
{
    ...
    public function execute(Survey $survey, array $schema): void
    {
        ...
        $managedKeys = collect($pages)
            ...
            ->values()
            ->all();
        $currentPageKeys = collect($pages)
            ->pluck('id')
            ->filter()
            ->map(fn (mixed $pageKey): string => (string) $pageKey)
            ->values()
            ->all();
        $managedPageIds = $survey->pages()
            ->whereIn('page_key', $currentPageKeys)
            ->pluck('id')
            ->all();
        ...
        $survey->fields()
            ...
            ->where(function ($query) use ($managedPageIds): void {
                ...
                if ($managedPageIds !== []) {
                    $query->orWhere(function ($query) use ($managedPageIds): void {
                        $query->where('is_hidden', true)
                            ->whereIn('survey_page_id', $managedPageIds);
                    });
                }
            })
            ->delete();
